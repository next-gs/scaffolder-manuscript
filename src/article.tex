...
\begin{abstract} %{{{1
  ...
  neighbouring sequence regions together and fill gaps \remove{in the assembly
  using additional sequences}. This scaffolding step\remove{, however,} is
  ...
  may make reproducing or editing an existing genome \change{build}{scaffold}
  difficult.
  ...
  regions to be \change{defined}{specified} and add additional sequences to
  fill \add{known} gaps in the scaffold. Defining the genome construction in
  a file makes the scaffolding process reproducible and easier to edit
  compared with \add{large} FASTA nucleotide sequences.
...
the many individual reads\remove{ that represent assembly into the longest
possible sequences, given the data. Nonetheless, }These contigs represent
a fragmented picture of the genome and \add{therefore} require additional work
...
a genome} scaffold \change{may} use wet laboratory methods, \emph{in silico}
...
\remove{and} The Scaffolder \change{allows}{format provides for using} these
additional insert sequences to \remove{be used} to fill \remove{these} gaps.
...
super-sequence. \add{Each scaffold file represents one scaffolded nucleotide
sequence and separate scaffolds should be defined in separate files.} \pb
...
In the putative scaffold shown in Figure 1 this completes the \remove{scaffold
} sequence. \pb

The final entry in the scaffold \remove{also} includes the ``inserts'' tag to
...
hard to reproduce and introduces \remove{the possibility of }irreducible
\change{techniques}{edits} and/or human error. In respect to \change{these
drawbacks}{this} the aims of the Scaffolder software are twofold: 1) to provide
...
whilst being succinct and readable. \change{The goal of this syntax is to make
genome scaffolds easy to write in a common text editor.}{The AGP format is
a similar format for describing scaffolds. This format can be used to define
contig order and N-filled gap regions in a scaffold. The advantages of the AGP
format is that each contig entry is defined on a single line which allows
searching the scaffold using UNIX command line tools. The scaffolder format in
comparison in contrast is written in the standardised YAML format and therefore
...
sequence of scaffold specified by in the file.} \pb
...
\add{The example use case for scaffolder is a combination of computation and
manual editing of a genome scaffold. As discussed many programming languages
provide libraries for reading and writing YAML. We have used scaffolder in our
genome projects create a scaffold file from parsing the output of \emph{in
silico} scaffolding tools into the scaffold file format. This scaffold was then
manually updated in a text editor with the results of using PCR to identify
gaps sequences. We see this as the advantage of describing a scaffold in both
a human and machine readable format.} \pb
