...
\subsection*{Comparison with other scaffold formats} %{{{2
...
to define contig order and N-filled gap regions in a scaffold. The advantages
of the AGP format is that each contig entry is defined on a single line which
allows searching the scaffold using UNIX line-based tools. The scaffolder
format in comparison in contrast is written in the standardised YAML format
and therefore accessible to the many languages which provide parsers to this
format. The scaffolder format is provided with a tool explicitly to produce
the fasta sequence of scaffold specified by in the file.} \pb
...
\subsection*{Example Use Case} %{{{2

\add{The example use case for scaffolder is a combination of computation and
manual editing of a genome scaffold. We have used scaffolder in our genome
projects to create a scaffold file from parsing the output of \emph{in silico}
scaffolding tools into the scaffold file format. This scaffold was then
manually updated in a text editor to fill gap regions with laboratory-produced
sequences.} \pb
