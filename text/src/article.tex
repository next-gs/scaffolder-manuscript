...
Scaffolder is a tool aimed at build genome sequence scaffolds through simple to
edit configuration files. Scaffolder allows contigs to be joined together
through defintition in the scaffold file. Each contigs is stored in a fasta
file, as provided by the standard output from an assembler such as
newbler\cite{newbler} or abyss\cite{abyss}. Scaffolder allows inserts to be
added to scaffold so that gaps in the assembled data may be bridged and filled
- a common process in finishing a genome sequence. Scaffolder also allows the
specification of unknown regions in the scaffold. These may be regions where
the sequence is unknown but the approximate size can be estimated from the
sequencing of insert pairs or from estimating distances from contigs aligned to
reference genomes.
...
exit statuses and providing manual pages for each of the commands.
