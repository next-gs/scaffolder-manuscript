...
\subsection*{Processing the scaffold file} %{{{2
...
scaffold file may have attributes which define whether a sub-sequence or the
reverse complement of the sequence should be used is the scaffold. The types of
attributes are outlined in the Results and Discussion section below.\pb

As the scaffold file is processed a corresponding ruby object instance is
created to correspond to an entry in the scaffold file. as an instance of
a subclass of Scaffolder::Region.  Scaffolder::Region is the super class for
all region entries in the scaffold.  This Region class provides the helper
methods for sub-sequencing and reversing complementing a sequence.  The Region
class also provides a class annotation method\cite{class-annotations} which
simplifies defining the attributes for each scaffold entry. Possible attributes
for scaffold entries can be added by using this helper method. \pb

An noteworthy method of the Scaffolder::Region class is the ``raw_sequence''
method. In the Scaffolder::Region class method returns by the sequence defined
by the ``source'' tag in the scaffold file. This sequence is then sub-sequenced
or reversed complemented if necessary. Subclasses of the Scaffolder::Region
class override this method to additional manipulate the nucleotide sequence.
For example, the Scaffolder::Region::Sequence class overrides the sequence
method to insert any specified inserts specified in the scaffold file before
this sequence is returned up the method chain.\pb
